...
TEST(emuTest, emuTest_cpuTestSTAX_B) {
    ...
    printf("Executing STAX B with A=0x%02X, BC=0x%04X\n", cpu.examineRegisterA(), cpu.examineRegisterBC());
    printf("Before STAX B: Memory[0x%04X] = 0x%02X\n", cpu.examineRegisterBC(), cpu.readByte(cpu.examineRegisterBC()));
    cpu.execute(opcode); // Execute STAX B instruction
    printf("After STAX B: Memory[0x%04X] = 0x%02X\n", cpu.examineRegisterBC(), cpu.readByte(cpu.examineRegisterBC()));
